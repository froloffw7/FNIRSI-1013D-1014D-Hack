...
//----------------------------------------------------------------------------------------------------------------------------------
static void allwinner_sdhost_update_irq(F1C100S_MMC *s)
{
    uint32_t irq;
    
    s -> irq_bits = s->irq_status.m_32bit;
    if (s->global_ctl.m_32bit & SD_GCTL_INT_ENB) {
        irq = s->irq_status.m_32bit & s->irq_mask.m_32bit;
    } else {
        irq = 0;
    }
    if (s->global_ctl.m_32bit & SD_GCTL_DMA_ENB) {
        s -> dma_irq_bits = s -> dmac_status.m_32bit;// & registers -> dmac_irq.m_32bit;
    } else {
        s -> dma_irq_bits = 0;        
    }

    trace_allwinner_sdhost_update_irq(s -> irq_bits, s -> dma_irq_bits);
...
static void allwinner_sdhost_reset(F1C100S_MMC * s) {
    ...
    s -> dmac_status.m_32bit = REG_IDST_RST;
...
void * F1C100sMMC0(PARMV5TL_CORE core, uint32_t address, uint32_t mode) {
    //Call the MMC handler with the registers for MMC0
    f1c100s_log("%08x ", *core->program_counter);
...
void F1C100sMMCRead(F1C100S_MMC * registers, uint32_t address, uint32_t mode) {
    ...
    case SD_STAR:
        /* Status */
        if (registers -> global_ctl.m_32bit & SD_GCTL_FIFO_AC_MOD) {
            if (sd_data_ready(registers -> sd)) {
                registers -> status.m_32bit &= ~SD_STAR_FIFO_EMPTY;
                registers -> status.m_32bit |= SD_STAR_FIFO_LEVEL_1;
                if (registers -> transfer_cnt >= 64*4)
                    registers -> status.m_32bit |= SD_STAR_FIFO_FULL;
                else
                    registers -> status.m_32bit &= ~SD_STAR_FIFO_FULL;
            } else {
                registers -> status.m_32bit |= SD_STAR_FIFO_EMPTY;
        ...
        f1c100s_log("->(%x)[%d]\n", registers -> status.m_32bit, registers -> transfer_cnt);
        break;
    case SD_HWRST:
    ...
    case SD_FIFO:
        if (registers -> global_ctl.m_32bit & SD_GCTL_FIFO_AC_MOD) {
            /* Read FIFO */
            if (sd_data_ready(registers -> sd)) {
                registers -> fifo.m_8bit[0] = sd_read_byte(registers -> sd);
                registers -> fifo.m_8bit[1] = sd_read_byte(registers -> sd);
                registers -> fifo.m_8bit[2] = sd_read_byte(registers -> sd);
                registers -> fifo.m_8bit[3] = sd_read_byte(registers -> sd);
                f1c100s_log("->[%02x %02x %02x %02x]\n",
                    registers -> fifo.m_8bit[0], registers -> fifo.m_8bit[1],
                    registers -> fifo.m_8bit[2], registers -> fifo.m_8bit[3]);
                allwinner_sdhost_update_transfer_cnt(registers, 4);
                allwinner_sdhost_auto_stop(registers);
                allwinner_sdhost_update_irq(registers);
            } else {
                f1c100s_log_mask(LOG_GUEST_ERROR, " No data ready on SD bus\n");
            }
        } else {
            registers -> fifo.m_32bit = 0;
            f1c100s_log_mask(LOG_GUEST_ERROR, " FIFO not enabled\n");
        }
        break;
    default:
        f1c100s_log("->\n");
...
void F1C100sMMCWrite(F1C100S_MMC * registers, uint32_t address, uint32_t mode) {
    ...
    case SD_BYCR:
        ...
        registers->transfer_cnt = registers -> byte_count.m_32bit;
        break;
    case SD_CMDR:
    ...
    case SD_RISR:
        f1c100s_log("<-(%x)\n", registers -> irq_status.m_32bit);
        registers -> irq_bits &= ~(registers -> irq_status.m_32bit);
        registers -> irq_status.m_32bit = registers -> irq_bits;
        break;
    case SD_FWLR:
        /*FIFO Water Level */
    ...
    case SD_IDST:
        /*Internal DMA Controller Status */
        f1c100s_log("<-(%x)\n", registers -> dmac_status.m_32bit);
        registers -> dma_irq_bits &= ~(registers -> dmac_status.m_32bit);
        registers -> dmac_status.m_32bit = registers -> dma_irq_bits;
        break;

    case SD_IDIE:
        /*Internal DMA Controller IRQ Enable */
        f1c100s_log("<-(%x)\n", registers -> dmac_irq.m_32bit);
        break;

    case SD_FIFO:
        if (registers -> global_ctl.m_32bit & SD_GCTL_FIFO_AC_MOD) {
            if (sd_receive_ready(registers -> sd)) {
                /* Write FIFO */
                f1c100s_log("<-[%02x %02x %02x %02x]\n",
                    registers -> fifo.m_8bit[0], registers -> fifo.m_8bit[1],
                    registers -> fifo.m_8bit[2], registers -> fifo.m_8bit[3]);
                value = registers -> fifo.m_32bit;
                sd_write_byte(registers -> sd,  value        & 0xFF);
                sd_write_byte(registers -> sd, (value >>  8) & 0xFF);
                sd_write_byte(registers -> sd, (value >> 16) & 0xFF);
                sd_write_byte(registers -> sd, (value >> 24) & 0xFF);
                allwinner_sdhost_update_transfer_cnt(registers, 4);
                allwinner_sdhost_auto_stop(registers);
                allwinner_sdhost_update_irq(registers);
            } else {
                f1c100s_log_mask(LOG_GUEST_ERROR, " SD bus receiver not ready\n");
            }
        } else {
            registers -> fifo.m_32bit = 0;
            f1c100s_log_mask(LOG_GUEST_ERROR, " FIFO not enabled\n");
